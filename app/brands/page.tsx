...
export default function BrandsPage() {
  ...
  const deleteBrand = async (id: number, name: string) => {
    if (!confirm(`Delete "${name}"? All scheduled posts, content, and analytics for this brand will be permanently lost. This cannot be undone.`)) return;
    setActing(id);
    try {
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <button onClick={loadBrands} className="bg-gray-800 hover:bg-gray-700 text-white text-sm px-4 py-2 rounded-lg transition">Retry</button>
        </div>
      ) : brands.length === 0 ? (
        ...
        <div className="space-y-3">
          {brands.map((b) => {
            const isCurrent = b.id === client?.campaign_id;
            return (
              <div key={b.id}
                className={`bg-gray-900 border rounded-xl p-5 flex items-center justify-between gap-4 transition ${
                  isCurrent ? "border-indigo-600/60" : b.suspended ? "border-amber-700/50 opacity-75" : "border-gray-800"
                }`}>
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-white font-semibold truncate">{b.name}</p>
                    {isCurrent && (
                      <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 font-medium flex-shrink-0">● Current</span>
                    )}
                    {b.suspended ? (
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {isCurrent ? (
                    <span className="px-3 py-1.5 bg-gray-800 text-gray-500 text-xs font-semibold rounded-lg cursor-default">
                      Active
                    </span>
                  ) : (
                    <button
                  ...
                  {b.suspended ? (
                    ...
                    <button
                      ...
                      disabled={acting === b.id || isCurrent}
